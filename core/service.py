...
imgkit = staticFunctions.imgkit

model = staticFunctions.model
globalConfig = staticFunctions.globalConfig
datetime = staticFunctions.datetime
...
class staticService():
    ...
    def getHelp(self, messageList:list[str],qid:str):
        ...
        try:
            ...
            return '您的指令错误！'
        imgkit.from_string(html,'../go-cqhttp/data/images/help.png',config=globalConfig.imgkit_config,css='./style.css')
    ...
    def getStats(self, messageList:list[str],qid:str):
        ...
        moneyData=[]
        fuelData=[]
        nowdate=staticFunctions.getnowdate()
        for i in range(6,-1,-1):
            date=nowdate-86400*i
            stats = model.Statistics.findAll(globalConfig.mysql, where='timestamp>=? and timestamp<=?', args=[date,date+86400])
        ...
        for datum in moneyData:
            ...
            ys.append(datum[1])
        globalConfig.plt.figure(figsize=(10,5))
        globalConfig.plt.plot(xs,ys,linestyle='-',marker=',',label='矿石',alpha=0.5)

        xs,ys=[],[]
        for datum in fuelData:
            xs.append(datetime.fromtimestamp(datum[0]-8*3600))
            ys.append(datum[1])
        globalConfig.plt.plot(xs,ys,linestyle='-',marker=',',label='燃油',alpha=0.5)

        globalConfig.plt.legend()
        globalConfig.plt.savefig('../go-cqhttp/data/images/statistics.png')
    ...
    def showWealthiest(self, messageList: list[str], qid: str):
        ...
        """
        ans = ""
        allUserList = model.User.findAll(globalConfig.mysql)
        allUserList.sort(key=staticFunctions.assetCalculation, reverse=True)
        showNum = round(0.1 * len(allUserList) + 1)
        for i in range(showNum):
            ans += "%s. %s拥有流动资产 %.2f 元\n" % (i+1, allUserList[i].qid, staticFunctions.assetCalculation(allUserList[i]))
...
class userService():
    def signup(self, messageList:list[str],qid:str):
        ...
        nowtime = staticFunctions.getnowtime()
        assert not model.User.find(qid, globalConfig.mysql) and not model.User.findAll(globalConfig.mysql, 'schoolID=?', (schoolID,)),'注册失败:您已经注册过，无法重复注册！'
    ...
    def getUserInfo(messageList:list[str],qid:str):
        ...
        :return: 查询提示信息
        """
        user:model.User = model.User.find(qid, globalConfig.mysql)
        if not user:
            return "[错误] 您尚未注册!"
        ...
        digable:str='是' if user.forbidtime[0] < staticFunctions.getnowtime() else '否'
        ...
        eres:str=''    #生产效率信息
        for index in range(globalConfig.effisItemCount):
            effis.setdefault(index,0.0)
            eres += globalConfig.effisStr[index]+":%.4f%%\n" % (staticFunctions.sigmoid(effis[index])*100)

        mineres:str='' #私有矿井信息
        for mine in mines:
            mineres+='%s,' % mine

        ans:str = globalConfig.infoMsg%(qid,schoolID,money,industrialTech,extractTech,refineTech,digable,
    ...
    def pay(messageList:list[str],qid:str):
        ...
        user.money-=money
        tuser.money+=round(money*(1-globalConfig.playerTax))
    ...
    def factorsLookup(messageList:list[str],qid:str):
        ...
        """
        user: User = User.find(qid, mysql)
        indicatorList = staticFunctions.indicators(user.schoolID)
        ans = '您的天赋代码的%s个判定段和其质因子如下：\n' % len(indicatorList)
        allFactorList = []
        for i in indicatorList:
            primeFactors, Factors = staticFunctions.factors(i)
            ans += '%s:' % i
    ...
    def learnEffis(messageList:list[str],qid:str):
        ...
        assert messageList[1] in globalConfig.effisNameDict, '学习失败：未知效率类别！'
        effisID = globalConfig.effisNameDict[messageList[1]]
        ...
        staticFunctions.send(tuser.qid,"%s(%s)向你学习了%s效率，您得到了%.2f元报酬，当前该项效率为%.2f点" %
